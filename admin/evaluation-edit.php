...
// Load evaluation
$stmt = $conn->prepare("SELECT * FROM player_evaluation WHERE id = ?");
...
// If coach, ensure evaluation belongs to them
if ($role === 'coach') {
    ...
    $coachId = ($u && !empty($u['coach_id'])) ? intval($u['coach_id']) : 0;
    ...
    if ($student_id <= 0 || $coach_id <= 0) {
        ...
        if (!$stmtU) {
            ...
            if ($stmtU->execute()) {
                ...
                $message = "Database error: " . $stmtU->error;
            }
        }
    }
}
    ...
    <form method="POST">
                ...
                <select name="student_id" required>
                    ...
                    <?php if ($students_res): ?>
                        <?php while ($s = $students_res->fetch_assoc()): ?>
                            <option value="<?php echo (int)$s['id']; ?>" <?php if ($s['id'] == $ev['student_id']) echo 'selected'; ?>>
                ...
                <select name="coach_id" required>
                    ...
                    <?php if ($coaches_res): ?>
                        <?php while ($c = $coaches_res->fetch_assoc()): ?>
                            <option value="<?php echo (int)$c['id']; ?>" <?php if ($c['id'] == $ev['coach_id']) echo 'selected'; ?>>
        ...
        <div class="form-grid-2">
            <div class="form-group">
                ...
                <input type="number" name="batting_rating" min="1" max="10" value="<?php echo htmlspecialchars($ev['batting_rating']); ?>">
            </div>
            <div class="form-group">
                <label>Bowling (1–10)</label>
                <input type="number" name="bowling_rating" min="1" max="10" value="<?php echo htmlspecialchars($ev['bowling_rating']); ?>">
            </div>
        </div>
        ...
        <div class="form-grid-2">
            <div class="form-group">
                ...
                <input type="number" name="fielding_rating" min="1" max="10" value="<?php echo htmlspecialchars($ev['fielding_rating']); ?>">
            </div>
            <div class="form-group">
                <label>Fitness (1–10)</label>
                <input type="number" name="fitness_rating" min="1" max="10" value="<?php echo htmlspecialchars($ev['fitness_rating']); ?>">
            </div>
        </div>
        ...
        <div class="form-grid-2">
            <div class="form-group">
                ...
                <input type="number" name="discipline_rating" min="1" max="10" value="<?php echo htmlspecialchars($ev['discipline_rating']); ?>">
            </div>
            <div class="form-group">
                <label>Attitude (1–10)</label>
                <input type="number" name="attitude_rating" min="1" max="10" value="<?php echo htmlspecialchars($ev['attitude_rating']); ?>">
            </div>
        </div>
